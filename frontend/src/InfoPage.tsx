...
import { formatDate, permutationIncreaseTest } from './utils'
import './InfoPage.css'
...
function pooledClimateResult(study: EventStudyData) {
  const values = study.effects
    .filter((effect) => effect.scope === 'affected' && effect.topicId === 'climate_change' &&
      effect.windowDays === 14 && effect.timing === 'onset' && effect.complete && !effect.overlap &&
...
function deriveCountrySignals(studies: EventStudyData[], labels: Record<string, string>): CountrySignal[] {
  ...
  for (const study of studies) {
    const effects = new Map(study.effects
      .filter((effect) => effect.scope === 'affected' && effect.topicId === 'climate_change' &&
        effect.windowDays === 7 && effect.timing === 'persistence')
      .map((effect) => [effect.eventId, effect]))
    const series = new Map(study.series.map((item) => [
      `${item.eventId}:${item.scope}:${item.topicId}:${item.timing}`,
      new Map(item.points.map((point) => [point[0], point[1]])),
    ]))

    for (const event of study.events) {
      const beforeSeries = series.get(`${event.id}:affected:climate_change:onset`)
      const afterSeries = series.get(`${event.id}:affected:climate_change:persistence`)
      if (!beforeSeries || !afterSeries) continue
      const before = Array.from({ length: 7 }, (_, index) => beforeSeries.get(index - 7))
      const after = Array.from({ length: 7 }, (_, index) => afterSeries.get(index))
      if (before.some((value) => value == null) || after.some((value) => value == null)) continue
      const result = permutationIncreaseTest(before as number[], after as number[])
      if (!result) continue
      const significant = result.difference > 0 && result.pValue < 0.05

      for (const country of event.geographyIds) {
        const summary = countries.get(country) ?? {
          tested: new Set<string>(), significant: new Set<string>(), overlapSignals: 0, changes: [], pValues: [],
        }
        summary.tested.add(event.id)
        if (significant) {
          summary.significant.add(event.id)
          summary.changes.push(result.percentChange ?? 0)
          summary.pValues.push(result.pValue)
          if (effects.get(event.id)?.overlap) summary.overlapSignals += 1
        }
        countries.set(country, summary)
      }
    }
  }

  return [...countries.entries()]
    .filter(([, result]) => result.significant.size > 0)
    .map(([id, result]) => ({
      id,
      label: labels[id] || id.replaceAll('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()),
      significantEvents: result.significant.size,
      testedEvents: result.tested.size,
      overlapSignals: result.overlapSignals,
      minimumChange: Math.min(...result.changes),
      maximumChange: Math.max(...result.changes),
      bestPValue: Math.min(...result.pValues),
    }))
    .sort((left, right) => right.significantEvents - left.significantEvents || left.bestPValue - right.bestPValue)
}

const formatPercent = (value: number | null) => value == null ? '—' : `${value >= 0 ? '+' : ''}${value.toFixed(1)}%`
const formatPValue = (value: number) => value < 0.001 ? '<0.001' : value.toFixed(3)
...
export default function InfoPage({
  ...
  const studyResults = useMemo(() => studies.map(pooledClimateResult).sort((left, right) => left.year - right.year), [studies])
  const countrySignals = useMemo(() => deriveCountrySignals(studies, manifest.geographyLabels), [manifest.geographyLabels, studies])
    ...
    <main className="info-view">
      <section className="info-hero">
        <div className="info-hero-copy">
          ...
          <h1>How extreme events move climate attention.</h1>
          <p>Climate Attention Atlas connects an independent record of major events with daily news publishing to ask when climate and electric-vehicle attention rises, where it rises, and whether it lasts.</p>
      ...
      <section className="info-snapshot" aria-label="Current project snapshot">
        <div><strong>{news?.geographyCount.toLocaleString() ?? '—'}</strong><span>publishing markets</span></div>
      ...
      <div className="info-content">
        <section className="info-question-grid">
          ...
          <article className="info-method-card">
            ...
            <ol>
              <li><span>1</span><p><strong>Define events independently.</strong> GDACS identifies the event before news attention is measured.</p></li>
        ...
        <section className="info-section" aria-labelledby="info-sources-heading">
          <header className="info-section-heading"><div><span className="eyebrow">Evidence base</span><h2 id="info-sources-heading">Independent streams, joined by place and time.</h2></div><p>Keeping event definition separate from news measurement avoids selecting only disasters that already received heavy coverage.</p></header>
        ...
        <section className="info-section info-findings" aria-labelledby="info-findings-heading">
          ...
          <div className="info-country-panel">
            <div className="info-country-copy">
              <span className="eyebrow">Promising country signals</span>
              <h3>Spain stands out, with an overlap caveat.</h3>
              <p>The current audit applies the interface's exact, one-sided 7-day post-event permutation test to every testable major event. These are nominal event-level flags, not multiple-testing-adjusted country effects.</p>
            </div>
            <div className="info-country-list">
              {countrySignals.map((country) => (
                <article key={country.id}>
                  <div><strong>{country.label}</strong><span>{country.significantEvents} of {country.testedEvents} event{country.testedEvents === 1 ? '' : 's'}</span></div>
                  <p>{country.minimumChange === country.maximumChange ? formatPercent(country.minimumChange) : `${formatPercent(country.minimumChange)} to ${formatPercent(country.maximumChange)}`} · best p {formatPValue(country.bestPValue)}</p>
                  {country.overlapSignals > 0 && <small>{country.overlapSignals} significant record{country.overlapSignals === 1 ? '' : 's'} overlap another same-country event</small>}
                </article>
              ))}
            </div>
          </div>
          <div className="info-caution"><ShieldCheck size={17} /><p><strong>Interpret carefully.</strong> Spain's repeated signal is driven by overlapping wildfire records, especially the August 2025 cluster; the other named countries each have one flagged event. Results remain sensitive to event, period, media scope, and coverage.</p></div>
        </section>
