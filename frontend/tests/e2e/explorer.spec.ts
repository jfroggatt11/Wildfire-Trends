...
test('attention toggle and before-after analysis share the selected measure', async ({ page }) => {
  ...
  await expect(page.locator('.event-drawer')).toBeVisible()
  await expect(page.locator('.severity-badge')).toContainText('Burned area: 11,161 ha')
  ...
  await expect(cards.first()).toContainText('Descriptive change')
...
test('info page explains the project, evidence, interface, and roadmap', async ({ page }) => {
  ...
  await expect(page.locator('.info-snapshot')).toContainText('182')
  ...
  await expect(page.getByRole('heading', { name: 'Does the pattern hold at 7, 14 and 28 days?' })).toBeVisible()
  await expect(page.locator('.info-country-list')).toContainText('28 days')
  await expect(page.locator('.info-country-list')).toContainText('Same events:')
...
test('analysis lab runs the major-event study for climate and electric vehicles', async ({ page }) => {
  ...
  await expect(page.locator('.country-comparison h3')).toHaveText('Largest responses for events affecting each country')
...
test('analysis lab exposes the all-alert activity and lag views', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Event load and attention over time' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Is the timing relationship consistent?' })).toBeVisible()
  await expect(page.locator('.activity-kpis > article')).toHaveCount(3)
  await expect(page.locator('.association-kpi')).toContainText('association')
  await expect(page.locator('.event-load-chart')).toBeVisible()
  await expect(page.locator('.attention-anomaly-chart')).toBeVisible()
  ...
  await expect(page.locator('.lag-facet')).toHaveCount(2)
  await expect(page.locator('.lag-direction-labels').first()).toContainText('Attention leads events')
  await expect(page.locator('.lag-direction-labels').first()).toContainText('Attention follows events')
  ...
  const comparisonLegend = page.locator('.activity-series-legend > span')
  ...
  await expect(page.locator('.lag-facet')).toHaveCount(4)
  const lagTicks = (await page.locator('.lag-facet').first().locator('.recharts-yAxis .recharts-cartesian-axis-tick-value').allTextContents()).map(Number).filter(Number.isFinite)
  expect(Math.max(...lagTicks.map(Math.abs))).toBeLessThanOrEqual(1)
...
test('analysis lab charts daily attention with selectable event markers', async ({ page }) => {
  ...
  const eventMarker = page.locator('.timeline-chart path.recharts-symbols').first()
  const chartSurface = await page.locator('.timeline-chart .recharts-surface').boundingBox()
  const markerX = Number(await eventMarker.getAttribute('cx'))
  const markerY = Number(await eventMarker.getAttribute('cy'))
  expect(chartSurface).not.toBeNull()
  await page.mouse.move(chartSurface!.x + markerX, chartSurface!.y + markerY)
  await expect(page.locator('.timeline-tooltip')).toBeVisible()
  await expect(page.locator('.timeline-tooltip')).toContainText('Matching articles')
  await expect(page.locator('.timeline-tooltip')).toContainText('event started')
  await page.getByLabel('Timeline lines').selectOption('wildfire_area')
  await expect(page.locator('.activity-state')).toHaveCount(0, { timeout: 30_000 })
  await expect(page.getByRole('heading', { name: 'Climate change and wildfire area in World' })).toBeVisible()
  await expect(page.getByLabel('Burn-area source')).toHaveValue('reported')
  await expect(page.locator('.timeline-series-key')).toContainText('GDACS reported event area · ha')
  ...
  await page.getByLabel('Burn-area source').selectOption('satellite')
  await expect(page.locator('.timeline-series-key')).toContainText('MODIS observed burned area · ha')
  await expect(page.getByText('No MODIS burned-area observations are exported yet.')).toBeVisible()
  await page.getByLabel('Burn-area source').selectOption('both')
  await expect(page.locator('.timeline-total')).toContainText('3 lines')
  await expect(page.locator('.timeline-series-key')).toContainText('GDACS reported event area · ha')
  await expect(page.locator('.timeline-series-key')).toContainText('MODIS observed burned area · ha')
  await page.getByLabel('Timeline lines').selectOption('vegetation')
  await expect(page.getByRole('heading', { name: 'Climate change and surface greenness/browning in World' })).toBeVisible()
  await expect(page.locator('.timeline-series-key')).toContainText('MODIS surface greenness/browning · NDVI anomaly')
  await expect(page.locator('.timeline-chart .recharts-yAxis')).toHaveCount(2)
  ...
  await expect(page.locator('.timeline-key')).not.toContainText('Green tier')
  ...
  await page.getByLabel('Open Event alert tier options').click()
  await page.getByRole('group', { name: 'Event alert tier' }).getByLabel('Green tier').check()
  ...
  await expect(page.getByLabel('Event type')).toBeDisabled()
  await page.getByLabel('Open Wildfire size options').click()
  const sizeOptions = page.getByRole('group', { name: 'Wildfire size' })
  await sizeOptions.getByLabel('Under 5,000 ha').uncheck()
  await sizeOptions.getByLabel('5,000–9,999 ha').uncheck()
  await sizeOptions.getByLabel('10,000–49,999 ha').uncheck()
  await expect(sizeOptions.getByLabel('50,000+ ha')).toBeChecked()
  await expect(page.locator('.timeline-key')).toContainText('50,000+ ha')
  await expect(page.locator('.timeline-key')).not.toContainText('Under 5,000 ha')
  ...
  await page.getByLabel('Timeline lines').selectOption('topics')
  await expect(page.locator('.activity-state')).toHaveCount(0, { timeout: 30_000 })
  await expect(page.getByRole('heading', { name: 'Topic comparison in 4-country group' })).toBeVisible()
  await expect(page.locator('.timeline-series-key > span')).toHaveCount(2)
  await expect(page.locator('.timeline-series-key')).toContainText('Climate change')
  await expect(page.locator('.timeline-series-key')).toContainText('Electric vehicles')

  await page.getByLabel('Time aggregation').selectOption('7')
  await expect(page.locator('.timeline-chart-card .eyebrow')).toHaveText('7-day rolling average')
  await page.getByLabel('Timeline lines').selectOption('countries')
  await expect(page.locator('.activity-state')).toHaveCount(0, { timeout: 30_000 })
  await expect(page.getByRole('heading', { name: 'Climate change across 4 countries' })).toBeVisible()
  await expect(page.locator('.timeline-series-key > span')).toHaveCount(4)
  ...
  await expect(page.locator('.lab-range-slider')).toHaveCount(1)
  await expect(page.locator('.lab-range-slider input[type="range"]')).toHaveCount(2)
...
test('Lab event drilldown and reload preserve the complete comparison specification', async ({ page }) => {
  const errors = collectClientErrors(page)
  await openExplorer(page)
  await page.getByRole('button', { name: 'Analysis Lab', exact: true }).click()
  await page.getByLabel('Event alert tier', { exact: true }).selectOption('major')
  await page.getByLabel(/Media group/).selectOption('other_eu27')
  await page.getByLabel('Chart topic').selectOption('electric_vehicles')
  await page.getByLabel('Chart measure').selectOption('political_share')
  await page.getByLabel('Comparison window').selectOption('28')
  await page.getByLabel('Timing', { exact: true }).selectOption('persistence')
  await expect(page.locator('.window-sensitivity')).toContainText('Same events across windows')
  await page.locator('.ranked-event-table > button').first().click()
  await expect(page.locator('.event-drawer')).toBeVisible()
  for (let pass = 0; pass < 2; pass += 1) {
    if (pass) await page.reload({ waitUntil: 'domcontentloaded' })
    await expect(page.locator('.scope-select select')).toHaveValue('other_eu27')
    await expect(page.getByLabel('Topic', { exact: true })).toHaveValue('electric_vehicles')
    await expect(page.getByLabel('Comparison window')).toHaveValue('28')
    await expect(page.getByLabel('Timing', { exact: true })).toHaveValue('persistence')
    await expect(page.getByRole('button', { name: 'Political share', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await expect(page).toHaveURL(/topic=electric_vehicles/)
    await expect(page).toHaveURL(/measure=political_share/)
    await expect(page).toHaveURL(/window=28/)
    await expect(page).toHaveURL(/timing=persistence/)
  }
  expect(errors).toEqual([])
})
